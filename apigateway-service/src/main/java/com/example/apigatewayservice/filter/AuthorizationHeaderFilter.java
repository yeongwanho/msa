...
public class AuthorizationHeaderFilter extends AbstractGatewayFilterFactory<AuthorizationHeaderFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return ((exchange, chain) -> {
            ...
            if (!request.getHeaders().containsKey(HttpHeaders.AUTHORIZATION)){
                return onErr(exchange,"no authorization header", HttpStatus.UNAUTHORIZED);
            }
            String authorizationHeader= request.getHeaders().get(org.springframework.http.HttpHeaders.AUTHORIZATION).get(0);
            String jwt = authorizationHeader.replace("Bearer", "");
            if (!isJwtValid(jwt)){
                return onErr(exchange,"JWT token is not valid", HttpStatus.UNAUTHORIZED);
            }
    ...
    //Mono,Flux -> Spring WebFlux // 단일값 mono
